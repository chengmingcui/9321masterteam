...
class HousesList(Resource):
    ...
    def post(self):
        ...
        return {
                   "HouseID": id,
                   "price": price,
                   "message": "The house {} information has been posted and predict price is {}".format(id, price)}, 200
...
class Houses(Resource):
    ...
    def put(self, id):
        ...
        for key in house:
            ...
            if key == "Type":
                if house[key] not in type_list:
                    return {"message": "Property \"{}\" value is invalid :\"{}\" ".format(str(key),house[key])}, 400
            if key == "Regionname":
                if house[key] not in region_list:
                    return {"message": "Property {}'s value is invalid".format(str(key))}, 400
...
@app.route("/")
def index():
    return redirect("/portal.html", code=302)
